...
#' @param extra_data Optional data frame used to produce extra plots. Defaults to `NULL`.
...
#' @param extra_prefix Character string prepended to each `plot_id` when constructing
#'   filenames and names in the returned environment for extra plots. Defaults to `"extra_"`.
#' @param extra_fun Function to call for extra plots.
#' @param session Optional `fundsr_session` object. Defaults to the package
#'   default session when `NULL`.
...
run_plots <- function(roll_diffs,
    ...
    ticker_map <- fundsr_get_option("ticker_map")
    ...
    purrr::pwalk(runs, function(plot_id,
        ...
        if (!is.null(extra_data)) {
            funds_translated <- ifelse(funds %in% names(ticker_map),
                                ticker_map[funds],
                                funds)
            ...
            if (!key %in% st$done_extra_sets) {
                ...
                st$done_extra_sets <- c(st$done_extra_sets, key)
            }
        }
    })
    plots_env
