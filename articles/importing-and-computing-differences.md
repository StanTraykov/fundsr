...
commonly used via the
[`import_fund()`](https://stantraykov.github.io/fundsr/reference/import_fund.md)
wrapper that extracts data from Excel files or directly for importing
...
fundsr can download Excel files via URLs you supply via
[`add_fund_urls()`](https://stantraykov.github.io/fundsr/reference/add_fund_urls.md).

``` r
add_fund_urls(c(
    AFUND = "https://www.fund-provider/afund.xlsx",
    ANTHR = "https://www.another-provider/anthr.xlsx"
))
```

Downloaded files are stored in the data directory (`fundsr.data_dir`
option) as `AFUND.xlsx`, etc. This allows for easy importing via
[`import_fund()`](https://stantraykov.github.io/fundsr/reference/import_fund.md).

### Retrieving benchmarks from fund files

Some fund files may include a benchmark return series in their Excel
downloads.
[`import_fund()`](https://stantraykov.github.io/fundsr/reference/import_fund.md)
can retrieve this benchmark via the `retrieve_benchmark` parameter.
...
**Note:** Benchmark series from fund files may have missing values.
...
A script setting up funds tracking INDEX1 could do the following.
...
add_data_loader(function() {
    ...
    import_fund("FUND1", benchmark = "INDEX1", ...)
    import_fund("FUND2", benchmark = "INDEX1", retrieve_benchmark = TRUE, ...)
    import_fund("FUND3", benchmark = "INDEX1", file = "fund3.xlsx", ...)
...
To call all registered data loaders and build the master series table by
joining all objects in the storage environment, use
[`build_all_series()`](https://stantraykov.github.io/fundsr/reference/build_all_series.md).
